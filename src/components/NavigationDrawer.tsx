...
export default function NavigationDrawer() {
  ...
  useEffect(() => {
    setMounted(true);
    // Preload categories on mount so they're ready when drawer opens
  ...
  const overlay = open ? (
    <div className="fixed inset-0 z-[60] flex">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-black/20 animate-[fadeIn_300ms_ease-out]"
        onClick={close}
      />

      {/* Drawer panel — slides in from left */}
      <div className="relative w-full max-w-sm bg-white flex flex-col animate-[slideInLeft_300ms_ease-out]">
        {/* Top bar */}
        <div className="relative flex items-center justify-between px-5 h-14 shrink-0">
          <button
            type="button"
